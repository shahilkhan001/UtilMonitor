...
class ElectricityActivity : AppCompatActivity() {
    ...
    private val electricityDataList = mutableListOf<Int>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tvElectricityData = findViewById<TextView>(R.id.tvElectricityData)
        val btnRefresh = findViewById<Button>(R.id.btnRefresh)
        etLimit = findViewById(R.id.etLimit)
        btnSaveLimit = findViewById(R.id.btnSaveLimit)
        chartView = findViewById(R.id.electricityCustomChart)

        // Save Limit button
        btnSaveLimit.setOnClickListener {
            val limit = etLimit.text.toString().toIntOrNull()
            if (limit != null) {
                PreferenceHelper.setLimit(this, "electricity_limit", limit)
        ...
        viewModel.electricityUsage.observe(this) { usage ->
            tvElectricityData.text = "Current Electricity usage: $usage kWh"
            electricityDataList.add(usage)
            chartView.setData(electricityDataList)

            val limit = PreferenceHelper.getLimit(this, "electricity_limit")
            if (limit != -1 && usage > limit) {
                checkNotificationPermissionAndNotify("Electricity Alert", "Usage exceeded $limit kWh")
            }
        }
